...
EOF
    ...
    if flatpak info "${FLATPAK_ID}" &> /dev/null; then
        ...
        error "Flatpak application is not accessible"
        exit 1
    fi
...
validate() {
    ...
    if [[ -z "${installed_info}" ]]; then
        error "Cannot retrieve flatpak info for ${FLATPAK_ID}"
        error "Remediation: Run 'flatpak install flathub ${FLATPAK_ID}' manually"
        exit 1
    fi
    ...
    if [[ "${installed_version}" == "${TOOL_VERSION}" ]]; then
        ...
        error "Remediation: Run 'flatpak update ${FLATPAK_ID}' or reinstall"
        exit 1
    fi
}
