...
verify_prerequisites() {
    ...
    local required_extensions=(
        "calendar" "ctype" "curl" "fileinfo" "gd" "iconv" "intl"
        "json" "ldap" "mbstring" "mysqli" "openssl" "pdo" "pdo_mysql"
        "session" "simplexml" "soap" "sockets" "sodium" "tokenizer"
...
install_tool() {
    ...
    wget -q --show-progress "$download_url" -O openemr.tar.gz
...
validate() {
    ...
    for file in "${critical_files[@]}"; do
        if [ ! -f "$INSTALL_DIR/$file" ]; then
            error "Critical file not found: $INSTALL_DIR/$file"
        fi
    done
